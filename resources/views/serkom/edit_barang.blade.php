...
<head>
    ...
    <title>WEB KELOLA BARANG - Edit Barang</title>
            ...
            <header class="mb-6">
                <div
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="text-base sm:text-lg md:text-xl font-bold text-gray-800 tracking-wide">Edit Data
                                Barang</h1>
                            <p class="text-xs text-gray-500 mt-0.5">Perbarui informasi barang inventaris</p>
                        </div>
                    </div>
                    <a href="{{ route('inventory') }}"
            ...
            <form action="{{ route('update.barang', $barang->id) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input
                            class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition"
                            type="text" id="nama_barang" name="nama_barang"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select id="kategori" name="kategori_barang"
                                ...
                                @php
                                    $selectedKategori = old('kategori_barang', $barang->kategori_barang);
                                @endphp
                                <option value="Elektronik" {{ $selectedKategori == 'Elektronik' ? 'selected' : '' }}>
                                    Elektronik</option>
                                <option value="Kabel & Aksesoris"
                                    {{ $selectedKategori == 'Kabel & Aksesoris' ? 'selected' : '' }}>
                                    Kabel & Aksesoris</option>
                                <option value="Fasilitas / Mebel"
                                    {{ $selectedKategori == 'Fasilitas / Mebel' ? 'selected' : '' }}>
                                    Fasilitas / Mebel</option>
                                <option value="Lainnya" {{ $selectedKategori == 'Lainnya' ? 'selected' : '' }}>
                                    Lainnya</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition"
                                type="number" id="stok" name="stok" min="0"
                                value="{{ old('stok', $barang->stok) }}" placeholder="Masukkan total unit" required>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="deskripsi" name="deskripsi" rows="3"
                            class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition"
                            placeholder="Contoh: Barang dalam kondisi baik, tersimpan di Lab Komputer 1">{{ old('deskripsi', $barang->deskripsi) }}</textarea>
                    </div>

                    <!-- Foto Barang -->
                    <div>
                        <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-1.5">Foto Barang</label>

                        <!-- Preview Foto Saat Ini -->
                        @if ($barang->image)
                            <div
                                class="mb-3 flex items-center gap-3 p-2.5 border border-gray-200 rounded-xl bg-gray-50">
                                <img src="{{ asset('storage/' . $barang->image) }}" alt="Foto Barang"
                                    class="w-14 h-14 object-cover rounded-lg border border-gray-200">
                                <span class="text-xs text-gray-500">Foto saat ini terpasang</span>
                            </div>
                        @endif

                        <input type="file" name="image"
                            class="block w-full text-xs sm:text-sm text-gray-500 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-amber-50 file:text-amber-700 hover:file:bg-amber-100 border border-gray-300 rounded-xl cursor-pointer bg-gray-50">
                ...
                <div class="pt-4 border-t border-gray-100 flex flex-col-reverse sm:flex-row justify-end gap-3">
                    ...
                    <button type="submit"
                        class="w-full sm:w-2/3 bg-amber-500 hover:bg-amber-600 text-white text-xs sm:text-sm font-semibold py-2.5 px-4 rounded-xl shadow-sm focus:ring-4 focus:ring-amber-200 transition flex items-center justify-center gap-2">
